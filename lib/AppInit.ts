...
export async function updateTasks(): Promise<void> {
	...
	for (const task of tasks) {
		//we use a for .. of loop instead of a forEach because we need to await the setting of a task before we set the next one, since forEach fires an asynchronous function for every item in the array
		await Task.set(task)
	}
}
