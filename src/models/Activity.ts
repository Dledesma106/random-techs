...
const Activity = {
    ...
    set: async (activity: IActivity) => {
        ...
        if ((await DB.read<IActivity>(collection, activity._id as string)) === null)
            return await DB.create(collection, activity);
    ...
    setUnsynched: async (activity: IActivity) => {
        if ((await DB.read<IActivity>(unsynched, activity._id as string)) === null)
            return await DB.create(unsynched, activity);
