...
export default function Task({ route, navigation }: { route: any; navigation: any }) {
	...
	async function savePicture(photoPath?: string) {
		...
		try {
			console.log(`${apiEndpoints.baseApiUrl}images`)
			const response = await axios.post(`${apiEndpoints.baseApiUrl}images?taskId=${(task as ITask)._id}`, formData, {
				headers: {
					'Content-Type': 'multipart/form-data'
				}
			})
		} catch (error) {
			console.log('error al mandar la foto')
			console.log(error)
		}
			...
			{showCamera ? (
				previewVisible ? (
					<View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
						<View>{photoPath && <ImageBackground source={{ uri: photoPath }} style={fullScreenImageStyle} />}</View>
				...
				<ScrollView className="bg-gray-300 h-screen pt-4">
					...
					<View className={card}>
						...
						<Pressable onPress={registerExpense}>
							<View
								className={`${taskExpenses.length > 0 ? cardItem : 'p-4'} flex flex-row items-center justify-between`}
							>
							...
							<View className="flex flex-col space-y-4 justify-between items-center">
								{currentTask.image.map((e) => (
									<View
										...
										key={e._id}
									>
										<Image source={{ uri: e.url, width: 1080, height: 1920 }} style={fullScreenImageStyle} />
									</View>
								))}
							</View>
						</View>
					...
					{photo?.map((e) => (
						<View>
							...
							>
								{e.url && <ImageBackground source={{ uri: e.url }} style={{ width: '100%', height: '100%' }} />}
							</View>
