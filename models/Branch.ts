...
const Branch = {
	...
	set: async (branch: IBranch) => {
		...
		branch.city = `${(branch.city as ICity).name}, ${((branch.city as ICity).province).name}`
		if (!(await DB.read(collection, branch._id))) return await DB.create(collection, branch)
	...
	setUnsynched: async (branch: IBranch) => {
		if (!(await DB.read(unsynched, branch._id))) return await DB.create(unsynched, branch)
