...
const Image = {
	...
	set: async (image: IImage) => {
		if (!(await DB.read(collection, image._id))) return await DB.create(collection, image)
	...
	setUnsynched: async (image: IImage) => {
		if (!(await DB.read(unsynched, image._id))) return await DB.create(unsynched, image)
