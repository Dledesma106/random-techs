...
const Image = {
    ...
    set: async(image:IImage)=>{
        if(!await DB.read(collection, image._id as string)) return await DB.create(collection, image)
    ...
    setUnsynched: async(image:IImage)=>{
        if(!await DB.read(unsynched, image._id as string)) return await DB.create(unsynched, image)
    ...
    markAsSynched: async(image:IImage) => {
        await DB.delete(unsynched, image._id as string)
        await DB.create(collection, image)
    },
    markAsUnsynched: async(image:IImage) => {
        await DB.delete(collection, image._id as string)
