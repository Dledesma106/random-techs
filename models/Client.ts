...
const Client = {
	...
	set: async (client: IClient) => {
		...
		if (!(await DB.read(collection, client._id))) return await DB.create(collection, client)
	...
	setUnsynched: async (client: IClient) => {
		if (!(await DB.read(unsynched, client._id))) return await DB.create(unsynched, client)
