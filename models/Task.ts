...
const Task = {
	...
	getAll: async () => {
		return await Promise.all(
			(await DB.getCollection<ITask>(collection)).map(async (task: ITask) => await Task.get(task._id as string))
		)
	},
	...
	getAllUnsynched: async () => {
		return await Promise.all(
			(await DB.getCollection<ITask>(unsynched)).map(async (task: ITask) => await Task.get(task._id as string))
		)
	},
